...
class Database:
    ...
    def add_order(self, user_id, order_type, recipient, details, amount_rub, payment_method, invoice_id=None):
        ...
        self.conn.commit()
        print(f"[DEBUG] Создан заказ #{order_id}, тип: {order_type}, сумма: {amount_rub} RUB")
        return order_id
    ...
    def update_order_status(self, order_id, status):
        ...
        print(f"[DEBUG] Статус заказа #{order_id} изменен на '{status}' (успех: {success})")
        return success
    ...
    def get_active_orders(self):
        ...
        try:
            ...
            print(f"[DEBUG] Найдено активных заказов: {len(orders)}")
            for order in orders:
                print(f"  - #{order[0]}: {order[2]} | {order[7]} | {order[5]} RUB")
            return orders
        except Exception as e:
            print(f"[DEBUG] Ошибка получения заказов: {e}")
            return []
    ...
    def get_order(self, order_id):
        ...
        if order:
            print(f"[DEBUG] Заказ #{order_id} найден в БД")
        else:
            print(f"[DEBUG] Заказ #{order_id} НЕ найден в БД")
        return order
...
async def db_check_command(message: types.Message):
    ...
    try:
        ...
        report = f"📊 **Отчет базы данных:**\n\n"
        report += f"👥 Пользователей: {users_count}\n"
        report += f"📦 Всего заказов: {orders_count}\n\n"
        
        if all_orders:
            report += "📋 **Список всех заказов:**\n"
            for order in all_orders:
                order_id, order_type, status, amount_rub = order
                report += f"#{order_id} | {order_type} | {status} | {amount_rub:.2f} RUB\n"
        else:
            report += "❌ Заказов нет в базе\n"
...
async def admin_active_orders_handler(callback: types.CallbackQuery):
    print(f"[DEBUG] admin_active_orders вызван пользователем {callback.from_user.id}")
    ...
    try:
        ...
        print(f"[DEBUG] Ошибка редактирования: {e}")
...
async def manage_order_handler(callback: types.CallbackQuery):
    print(f"[DEBUG] manage_order_ вызван с data: {callback.data}")
    ...
    try:
        ...
        print(f"[DEBUG] Парсинг order_id: {order_id}")
        ...
        if photo_file_id and status in ["waiting_confirmation", "confirmed"]:
            ...
            try:
                ...
                await callback.message.answer("⚠️ Не удалось загрузить фото оплаты")
        ...
        print(f"[DEBUG] Ошибка парсинга order_id: {e}")
...
async def main():
    ...
    print("👉 /dbcheck - проверка базы данных")
    print("👉 📦 Активные заказы - РАБОТАЕТ С ОТЛАДКОЙ")
    print("=" * 50)
